...
uninstall() {
  ...
  rm -f "$CLAUDE_DIR/hooks/claude-notify.sh" "$CLAUDE_DIR/claude-signal-watcher.sh" \
        "$CLAUDE_DIR/claudebar-focus.sh" "$CLAUDE_DIR/claudebar-update.sh" \
        "$CLAUDE_DIR/claudebar-update.json" \
...
ensure_jq   # before the version banner: reading package.json needs it

# What we are installing, and how to update it later. semantic-release owns
# package.json's version, so that file is the only place a version exists —
# and none of the installed scripts end up next to it. The value is stamped
# into the library instead (see below), which is where every script reads it
# back from and what the menu bar shows.
VERSION=$(jq -r '.version // ""' "$SCRIPT_DIR/package.json" 2>/dev/null || true)
[ -n "$VERSION" ] || VERSION="dev"
# A checkout can pull; anything else (npx, a downloaded tarball) is refreshed
# by running npx again, which needs no path to remember.
if [ -d "$SCRIPT_DIR/.git" ]; then INSTALL_METHOD="git"; else INSTALL_METHOD="npx"; fi

# Braces are load-bearing: bash 3.2 (macOS /bin/bash) counts the ellipsis'
# high bytes as name characters, so `$VERSION…` reads as a variable called
# "VERSION…" — unbound, and under set -u that aborts the install right here.
echo "Installing claudebar ${VERSION}…"
...
install -m 0755 "$SCRIPT_DIR/host/claudebar-update.sh" "$CLAUDE_DIR/claudebar-update.sh"
...
# The version stamp, generated rather than shipped — it is the one thing that
# differs between two installs of the same source tree. version.sh sources it
# from beside itself, so it rides into a sandbox on the same symlink.
...
}
{
  ...
  echo "CLAUDEBAR_VERSION=$(sh_quote "$VERSION")"
  echo "CLAUDEBAR_INSTALL_METHOD=$(sh_quote "$INSTALL_METHOD")"
  echo "CLAUDEBAR_INSTALL_SOURCE=$(sh_quote "$SCRIPT_DIR")"
} > "$LIB_DST/installed.sh"
chmod 0644 "$LIB_DST/installed.sh"
info "hook, watcher, focus/update scripts and shared lib installed"
